...
def call_build_config(config_path):
    ...
    try:
        ...
        if subdoms:
            config_builder = serve.build_config(logger, override_path=config_path, ssl={"type": "none"}, subdomains=subdoms)
        else:
            config_builder = serve.build_config(logger, override_path=config_path, ssl={"type": "none"})
...
def check_environ(product):
    if product not in ("android_weblayer", "android_webview", "chrome", "chrome_android", "firefox", "firefox_android", "servo"):
        logger.debug("CONFIG PATH " + os.path.join(wpt_root, "config.json"))
...
def setup_wptrunner(venv, **kwargs):
    ...
    if not venv.skip_virtualenv_setup:
        logger.debug("INSTALLING REQS FROM " + os.path.join(wptrunner_path, "requirements.txt"))
