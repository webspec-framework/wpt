...
def _merge_dict(base_dict, override_dict, logger=None):
    rv = base_dict.copy()
    for key, value in base_dict.items():
        if key in override_dict:
            if isinstance(value, dict):
                ...
                if logger: logger.debug(f"setting {key} to {override_dict[key]}")
                rv[key] = override_dict[key]

    return rv

def _real_merge_dict(base_dict, override_dict, logger=None):
    rv = base_dict.copy()
    for key, value in override_dict.items():
        if isinstance(value, dict):
            rv[key] = _merge_dict(value, override_dict[key], logger=logger)
        else:
            if logger: logger.debug(f"setting {key} to {override_dict[key]}")
            rv[key] = override_dict[key]
...
class ConfigBuilder:
    ...
    def update(self, override):
        ...
        for k in self._default:
            if k in override:
                self._logger.debug(f"setting {k}")
    ...
    def _set_override(self, k, v):
        ...
        if isinstance(old_v, dict):
            self._logger.debug("merging")
            # self._data[k] = _merge_dict(old_v, v, logger=self._logger)
            self._data[k] = _real_merge_dict(old_v, v, logger=self._logger)
        else:
