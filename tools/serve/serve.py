...
def build_config(logger, override_path=None, config_cls=ConfigBuilder, **kwargs):
    ...
    logger.debug(f"kwargs in build_config: {kwargs}")
    ...
    logger.debug(f"OVERRIDE PATH {override_path}")
    logger.debug(f"DATA1 {rv._data}")
    if override_path and os.path.exists(override_path):
        with open(override_path) as f:
            override_obj = json.load(f)
        rv.update(override_obj)

    logger.debug(f"DATA2 {rv._data}")
